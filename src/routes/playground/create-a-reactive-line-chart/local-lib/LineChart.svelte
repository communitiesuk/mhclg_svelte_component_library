<script>
  ...
  function updateSelectedAreaCode(event) {
    console.log(event.currentTarget.dataset);
    selectedAreaCode = event.currentTarget.dataset.id;
  }
</script>
        ...
        <g data-role="lines-group">
          {#each data.lines as line, i}
            ...
            <Line
              ...
              onMouseMove={updateSelectedAreaCode}
            />
          {/each}
          <Line
            {lineFunction}
            dataArray={data.lines.find((d) => d.areaCode === selectedAreaCode)
              .data}
            pathStrokeColor="red"
            pathStrokeWidth="5"
            opacity="1"
