<script module>
  ...
  let descriptionArray = [
    "Allows the user to select multiple tags. The component returns an objects which has items which have colour asigned to tem",
  ];

  let contextArray = [
    "Can be used where the user needs to select multiple things such as LAs, Towns, postcode which are used on the further components e.g Charts ",
  ];
...
<script>
  ...
  import BasicMultiSelect from "$lib/components/ui/BasicMultiSelect.svelte";
  import Examples from "./basic-multi-select/Examples.svelte";
    ...
    addIndexAndInitalValue([
      {
        ...
        description: {
          markdown: true,
          arr: ["Options can be passed in for the user to chose from"],
        },
      },
      {
        name: "selectedWithColor",
        category: "Input props",
        isBinded: true,
        value: selectedWithColors,
        description: {
          markdown: true,
          arr: [
      ...
      },
      {
        name: "colorArray",
        category: "Input props",
        value: ["#1d70b8", "#d4351c", "#00703c"],
        description: {
          markdown: true,
          arr: [
            "Colours array is used to assign colours to the tags, loops back to the first colour if tags are bigger then the colour array",
          ],
        },
      },
      {
        ...
        description: {
          markdown: true,
          arr: ["Placeholder text telling the user want to do"],
        },
      },
    ]),
  );
...
{#snippet Component()}
  <div class="p-8">
    <BasicMultiSelect {...parametersObject} bind:selectedWithColors
    ></BasicMultiSelect>
  </div>
{/snippet}
