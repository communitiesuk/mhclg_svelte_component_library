    ...
    addIndexAndInitalValue([
      ...
      },
      {
        ...
        description: {
          markdown: true,
          arr: [
            `Applies organisation-specific branding by adding a <code>brand--{brand-slug}</code> class to the main <code>nav</code> element and potentially other classes (like <code>brand__color</code> or <code>brand__border-color</code>) to child elements. Select a brand slug from the dropdown. Requires corresponding CSS rules within the component for the specified brand slug.`,
            `<strong>Note:</strong> When using the <code>alternativeLineStyle</code>, the active item's border will use the brand color.`,
          ],
        },
      },
      {
